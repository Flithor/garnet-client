...
// NOTE: StackExchange.Redis is NOT referenced in this project.
// The original test used StackExchange.Redis.SortedSetEntry as a data container.
// It has been replaced with a local (string Element, double Score) record.
// using StackExchange.Redis;

namespace Garnet.client.test
{
    /// <summary>
    /// Replacement for StackExchange.Redis.SortedSetEntry used in the original test.
    /// </summary>
    internal readonly record struct SortedSetEntry(string Element, double Score);

    [AllureNUnit]
    [TestFixture]
    public class RespSortedSetGarnetClientTests : AllureTestBase
    {
        // NOTE: Original test started an embedded Garnet server:
        ...
        // }
        //
        // Tests now connect to an already-running Garnet server.
        // Configure the endpoint via the GARNET_TEST_ENDPOINT environment variable (format: "host:port").
        ...
        {
            // Flush the database to ensure a clean state (replaces starting a fresh server per test)
        ...
        {
            using var db = new GarnetClient(TestConfig.GetEndPoint());
            db.Connect();
