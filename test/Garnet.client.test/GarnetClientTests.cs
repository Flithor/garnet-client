...
namespace Garnet.client.test
{
    ...
    public class GarnetClientTests : AllureTestBase
    {
        ...
        {
            // Flush all keys to start each test with a clean state
            // (replaces the original approach of starting a new server per test)
            using var db = TestConfig.GetGarnetClient();
        ...
        [Test]
        public void SetGetWithCallback()
        {
            // NOTE: TLS variant is not tested here as it requires server-side TLS configuration.
            // Original test: [Values] bool useTLS — replaced with non-TLS only.
        ...
        public void SimpleMetricsTest([Values] bool recordLatency)
        {
            ...
            using var db = TestConfig.GetGarnetClient(recordLatency: recordLatency);
            db.Connect();
        ...
        public async Task ShouldNotThrowExceptionForEmptyArrayResponseAsync()
        {
            ...
            ClassicAssert.DoesNotThrowAsync(async () =>
            {
                ...
                // After FLUSHDB in SetUp there should be no keys; result may be null or empty
            });
        }

        [Test]
        public async Task CanUseMGetTests()
        {
            // NOTE: disableObjectStore variant is not tested here — requires server restart with specific config.
            // Original test: [Values] bool disableObjectStore — always using default server config (objects enabled).
        ...
        // NOTE: MultipleSocketPing test is excluded from this project because it requires
        // a Garnet server configured with both TCP and Unix domain socket endpoints.
        // The original test is preserved in test/Garnet.test/GarnetClientTests.cs.
        //
        // [Test]
